...
object Att {
   ...
   */
  final val ALIAS = Key.builtin("alias", KeyParameter.Forbidden, Set(classOf[Production], classOf[Rule]))
  final val ALIAS_REC = Key.builtin("alias-rec", KeyParameter.Forbidden, Set(classOf[Production], classOf[Rule]))
  final val ALL_PATH = Key.builtin("all-path", KeyParameter.Forbidden, Set(classOf[Claim]))
  final val ANYWHERE = Key.builtin("anywhere", KeyParameter.Forbidden, Set(classOf[Rule]))
  final val APPLY_PRIORITY = Key.builtin("applyPriority", KeyParameter.Required, Set(classOf[Production]))
  final val ASSOC = Key.builtin("assoc", KeyParameter.Forbidden, Set(classOf[Production]))
  final val AVOID = Key.builtin("avoid", KeyParameter.Forbidden, Set(classOf[Production]))
  final val BAG = Key.builtin("bag", KeyParameter.Forbidden, Set(classOf[Production]))
  final val BINDER = Key.builtin("binder", KeyParameter.Optional, Set(classOf[Production]))
  final val BRACKET = Key.builtin("bracket", KeyParameter.Forbidden, Set(classOf[Production]))
  final val CELL = Key.builtin("cell", KeyParameter.Forbidden, Set(classOf[Production]))
  final val CELL_COLLECTION = Key.builtin("cellCollection", KeyParameter.Forbidden, Set(classOf[Production], classOf[SyntaxSort]))
  final val CELL_NAME = Key.builtin("cellName", KeyParameter.Required, Set(classOf[Production]))
  final val CIRCULARITY = Key.builtin("circularity", KeyParameter.Forbidden, Set(classOf[Claim]))
  final val COLOR = Key.builtin("color", KeyParameter.Required, Set(classOf[Production]))
  final val COLORS = Key.builtin("colors", KeyParameter.Required, Set(classOf[Production]))
  final val COMM = Key.builtin("comm", KeyParameter.Forbidden, Set(classOf[Production], classOf[Rule]))
  final val CONCRETE = Key.builtin("concrete", KeyParameter.Optional, Set(classOf[Module], classOf[Production], classOf[Rule]))
  final val CONSTRUCTOR = Key.builtin("constructor", KeyParameter.Forbidden, Set(classOf[Production]))
  final val CONTEXT = Key.builtin("context", KeyParameter.Required, Set(classOf[ContextAlias]))
  final val COOL = Key.builtin("cool", KeyParameter.Forbidden, Set(classOf[Rule]))
  final val DEPENDS = Key.builtin("depends", KeyParameter.Required, Set(classOf[Claim]))
  final val ELEMENT = Key.builtin("element", KeyParameter.Required, Set(classOf[Production]))
  final val EXIT = Key.builtin("exit", KeyParameter.Forbidden, Set(classOf[Production]))
  final val FORMAT = Key.builtin("format", KeyParameter.Required, Set(classOf[Production]))
  final val FRESH_GENERATOR = Key.builtin("freshGenerator", KeyParameter.Forbidden, Set(classOf[Production]))
  final val FUNCTION = Key.builtin("function", KeyParameter.Forbidden, Set(classOf[Production]))
  final val FUNCTIONAL = Key.builtin("functional", KeyParameter.Forbidden, Set(classOf[Production]))
  final val GROUP = Key.builtin("group", KeyParameter.Required, Set(classOf[Sentence]))
  final val HASKELL = Key.builtin("haskell", KeyParameter.Forbidden, Set(classOf[Module]))
  final val HEAT = Key.builtin("heat", KeyParameter.Forbidden, Set(classOf[Rule]))
  final val HOOK = Key.builtin("hook", KeyParameter.Required, Set(classOf[Production], classOf[SyntaxSort]))
  final val HYBRID = Key.builtin("hybrid", KeyParameter.Optional, Set(classOf[Production]))
  final val IDEM = Key.builtin("idem", KeyParameter.Forbidden, Set(classOf[Production]))
  final val IMPURE = Key.builtin("impure", KeyParameter.Forbidden, Set(classOf[Production]))
  final val INDEX = Key.builtin("index", KeyParameter.Required, Set(classOf[Production]))
  final val INITIAL = Key.builtin("initial", KeyParameter.Forbidden, Set(classOf[Production]))
  final val INITIALIZER = Key.builtin("initializer", KeyParameter.Forbidden, Set(classOf[Production], classOf[Rule]))
  final val INJECTIVE = Key.builtin("injective", KeyParameter.Forbidden, Set(classOf[Production]))
  final val INTERNAL = Key.builtin("internal", KeyParameter.Forbidden, Set(classOf[Production]))
  final val KAST = Key.builtin("kast", KeyParameter.Forbidden, Set(classOf[Module]))
  final val KLABEL = Key.builtin("klabel", KeyParameter.Required, Set(classOf[Production]))
  final val KORE = Key.builtin("kore", KeyParameter.Forbidden, Set(classOf[RuleOrClaim], classOf[Module]))
  final val LABEL = Key.builtin("label", KeyParameter.Required, Set(classOf[Sentence]))
  final val LATEX = Key.builtin("latex", KeyParameter.Required, Set(classOf[Production]))
  final val LEFT = Key.builtin("left", KeyParameter.Optional, Set(classOf[Production]))
  final val LOCATIONS = Key.builtin("locations", KeyParameter.Forbidden, Set(classOf[SyntaxSort]))
  final val MACRO = Key.builtin("macro", KeyParameter.Forbidden, Set(classOf[Production], classOf[Rule]))
  final val MACRO_REC = Key.builtin("macro-rec", KeyParameter.Forbidden, Set(classOf[Production], classOf[Rule]))
  final val MAINCELL = Key.builtin("maincell", KeyParameter.Forbidden, Set(classOf[Production]))
  final val MEMO = Key.builtin("memo", KeyParameter.Forbidden, Set(classOf[Production]))
  final val ML_BINDER = Key.builtin("mlBinder", KeyParameter.Forbidden, Set(classOf[Production]))
  final val ML_OP = Key.builtin("mlOp", KeyParameter.Forbidden, Set(classOf[Production]))
  final val MULTIPLICITY = Key.builtin("multiplicity", KeyParameter.Required, Set(classOf[Production]))
  final val NON_ASSOC = Key.builtin("non-assoc", KeyParameter.Forbidden, Set(classOf[Production]))
  final val NON_EXECUTABLE = Key.builtin("non-executable", KeyParameter.Forbidden, Set(classOf[Rule]))
  final val NOT_LR1 = Key.builtin("not-lr1", KeyParameter.Forbidden, Set(classOf[Module]))
  final val NO_EVALUATORS = Key.builtin("no-evaluators", KeyParameter.Forbidden, Set(classOf[Production]))
  final val ONE_PATH = Key.builtin("one-path", KeyParameter.Forbidden, Set(classOf[Claim]))
  final val OWISE = Key.builtin("owise", KeyParameter.Forbidden, Set(classOf[Rule]))
  final val PARSER = Key.builtin("parser", KeyParameter.Required, Set(classOf[Production]))
  final val PREC = Key.builtin("prec", KeyParameter.Required, Set(classOf[Production]))
  final val PREFER = Key.builtin("prefer", KeyParameter.Forbidden, Set(classOf[Production]))
  final val PRESERVES_DEFINEDNESS = Key.builtin("preserves-definedness", KeyParameter.Forbidden, Set(classOf[Rule]))
  final val PRIORITY = Key.builtin("priority", KeyParameter.Required, Set(classOf[Rule]))
  final val PRIVATE = Key.builtin("private", KeyParameter.Forbidden, Set(classOf[Module], classOf[Production]))
  final val PUBLIC = Key.builtin("public", KeyParameter.Forbidden, Set(classOf[Module], classOf[Production]))
  final val RESULT = Key.builtin("result", KeyParameter.Required, Set(classOf[ContextAlias], classOf[Context], classOf[Rule]))
  final val RETURNS_UNIT = Key.builtin("returnsUnit", KeyParameter.Forbidden, Set(classOf[Production]))
  final val RIGHT = Key.builtin("right", KeyParameter.Optional, Set(classOf[Production]))
  final val SEQSTRICT = Key.builtin("seqstrict", KeyParameter.Optional, Set(classOf[Production]))
  final val SIMPLIFICATION = Key.builtin("simplification", KeyParameter.Optional, Set(classOf[Rule]))
  final val SMTLIB = Key.builtin("smtlib", KeyParameter.Required, Set(classOf[Production]))
  final val SMT_HOOK = Key.builtin("smt-hook", KeyParameter.Required, Set(classOf[Production]))
  final val SMT_LEMMA = Key.builtin("smt-lemma", KeyParameter.Forbidden, Set(classOf[Rule]))
  final val STREAM = Key.builtin("stream", KeyParameter.Optional, Set(classOf[Production], classOf[Rule]))
  final val STRICT = Key.builtin("strict", KeyParameter.Optional, Set(classOf[Production]))
  final val STRUCTURAL = Key.builtin("structural", KeyParameter.Forbidden, Set(classOf[Rule]))
  final val SYMBOL = Key.builtin("symbol", KeyParameter.Forbidden, Set(classOf[Production]))
  final val SYMBOLIC = Key.builtin("symbolic", KeyParameter.Optional, Set(classOf[Module], classOf[Production], classOf[Rule]))
  final val TAG = Key.builtin("tag", KeyParameter.Required, Set(classOf[Rule]))
  final val TOKEN = Key.builtin("token", KeyParameter.Forbidden, Set(classOf[SyntaxSort], classOf[Production]))
  final val TOTAL = Key.builtin("total", KeyParameter.Forbidden, Set(classOf[Production]))
  final val TRUSTED = Key.builtin("trusted", KeyParameter.Forbidden, Set(classOf[Claim]))
  final val TYPE = Key.builtin("type", KeyParameter.Required, Set(classOf[Production]))
  final val UNBOUND_VARIABLES = Key.builtin("unboundVariables", KeyParameter.Required, Set(classOf[RuleOrClaim]))
  final val UNIT = Key.builtin("unit", KeyParameter.Required, Set(classOf[Production]))
  final val UNPARSE_AVOID = Key.builtin("unparseAvoid", KeyParameter.Forbidden, Set(classOf[Production]))
  final val UNUSED = Key.builtin("unused", KeyParameter.Forbidden, Set(classOf[Production]))
  final val WRAP_ELEMENT = Key.builtin("wrapElement", KeyParameter.Required, Set(classOf[Production]))
