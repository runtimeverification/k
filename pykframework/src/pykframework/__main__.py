...
from .cli.pykframework import PrintInput, create_argument_parser, generate_options, parse_toml_args
...
def main() -> None:
    ...
    # Most pykframework operations are defined recursively, meaning you get a callstack the same depth as the term.
    # This change makes it so that in most cases, by default, pykframework doesn't run out of stack space.
...
def exec_kompilex(options: KompileXCommandOptions) -> None:
    ...
    with NamedTemporaryFile('w', prefix='pykframework_kompilex_', delete=not options.debug) as ntf:
