...
//				System.out.println("\tPItem: " + production.getItems().get(i));
				...
				// if the sort of the j-th term is a list sort
					...
					if (!psort.equals(tsort))
					{
						if (isListSort(psort) && subsort(tsort, psort))
						{
							...
							System.out.println("Adding cons at " + tc.getLocation());
						}
						
						if (!subsort(tsort, psort) && isListSort(psort))
							Error.silentReport("Cannot infer list terminator for term:" + tc + " at location " + tc.getLocation() + " in file: " + tc.getFilename() + "\n    Expected sort: " + psort + " found sort: " + tsort);
					}
					j++;
				}
				i++;
