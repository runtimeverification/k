...
public class KoreBackend extends AbstractBackend {
    ...
    public Function<Definition, Definition> steps() {
        ...
        Function1<Definition, Definition> resolveFreshConstants = d ->
                DefinitionTransformer.from(m -> new ResolveFreshConstants(d, kompileOptions.topCell, files, kompileOptions.pedanticAttributes).resolve(m), "resolving !Var variables").apply(d);
    ...
    public Function<Module, Module> specificationSteps(Definition def) {
        ...
        Function1<Module, Module> resolveFreshConstants = d ->
                ModuleTransformer.from(new ResolveFreshConstants(def, kompileOptions.topCell, files, kompileOptions.pedanticAttributes)::resolve, "resolving !Var variables").apply(d);
