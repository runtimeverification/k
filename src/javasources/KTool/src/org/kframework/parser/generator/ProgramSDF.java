    ...
    public static StringBuilder getSdfForPrograms(Definition def, Context context) {
        ...
        for (Production p : psdfv.outsides) {
            if (p.isListDecl()) {
                ...
                sdf.append("    {" + StringUtil.escapeSortName(si.getSort()) + " " + StringUtil.escapeSDF(si.getSeparator()) + "}" + si.getListType() + " -> " + StringUtil.escapeSortName(p.getSort()));
        ...
        for (String t : ctv.terminals) {
            if (t.matches("[a-zA-Z\\_][a-zA-Z0-9\\_]*")) {
                sdf.append("    " + StringUtil.escapeSDF(t) + " -> IdDz {reject}\n");
            }
        }
        ...
        for (Production p : psdfv.lexical) {
            ...
            if (p.containsAttribute("regex")) {
                ...
                for (String t : ctv.terminals) {
                    ...
                    if (m.matches())
                        sdf.append("    " + StringUtil.escapeSDF(t) + " -> " + StringUtil.escapeSortName(p.getSort()) + "Dz {reject}\n");
                }
            } else {
        ...
        for (Restrictions r : psdfv.restrictions) {
            if (r.getTerminal() != null && !r.getTerminal().getTerminal().equals(""))
                sdf.append("    " + StringUtil.escapeSDF(r.getTerminal().getTerminal()) + " -/- " + r.getPattern() + "\n");
            else
