...
If you compare the running time of the just-in-time parser by running
`kast --output kore and-or.bool` to the running time of the ahead-of-time
parser by running `foo-kompiled/parser_PGM and-or.bool`, you will see that
indeed, one is much faster than the other. This is because the ahead-of-time
parser is able to make use of the knowledge of the grammar to generate an
efficient parser when you invoke `kompile`, thus optimizing the time spent
parsing an individual program.
...
2. Write a program where the meaning of the arithmetic expression based on
