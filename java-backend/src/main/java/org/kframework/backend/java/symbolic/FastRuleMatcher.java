...
public class FastRuleMatcher {
    ...
    private boolean matchOnFunctionSymbol = false;
    private TermContext context;

    public static ConjunctiveFormula unify(Term term, Term otherTerm, TermContext context) {
        return new FastRuleMatcher(context.global(), 1).unifyEquality(term, otherTerm, false, false, true, false, context);
    }
     ...
     */
    public ConjunctiveFormula unifyEquality(Term subject, Term pattern, boolean patternFold, boolean partialSimplification, boolean continuousSimplification, boolean matchOnFunctionSymbol, TermContext context) {
    ...
    private BitSet match(Term subject, Term pattern, BitSet ruleMask, scala.collection.immutable.List<Pair<Integer, Integer>> path) {
        ...
        if (subject.isSymbolic() && !matchOnFunctionSymbol) {
        ...
        if (pattern.isSymbolic() && !matchOnFunctionSymbol) {
            return addUnification(subject, pattern, ruleMask, path);
        }
