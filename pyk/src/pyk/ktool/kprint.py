...
def pretty_print_kast(kast: KAst, symbol_table: SymbolTable, debug=False):
    ...
    if type(kast) is KApply:
        ...
        args = kast.args
        unparsedArgs = [pretty_print_kast(arg, symbol_table, debug=debug) for arg in args]
        if kast.is_cell:
    ...
    if type(kast) is KAs:
        patternStr = pretty_print_kast(kast.pattern, symbol_table, debug=debug)
        aliasStr = pretty_print_kast(kast.alias, symbol_table, debug=debug)
        return patternStr + ' #as ' + aliasStr
    if type(kast) is KRewrite:
        lhsStr = pretty_print_kast(kast.lhs, symbol_table, debug=debug)
        rhsStr = pretty_print_kast(kast.rhs, symbol_table, debug=debug)
    ...
    if type(kast) is KProduction:
        ...
        sortStr = pretty_print_kast(kast.sort, symbol_table, debug=debug)
        if not kast.items:
            return 'syntax ' + sortStr
        productionStr = ' '.join([pretty_print_kast(pi, symbol_table, debug=debug) for pi in kast.items])
        attStr = pretty_print_kast(kast.att, symbol_table, debug=debug)
    ...
    if type(kast) is KSortSynonym:
        newSortStr = pretty_print_kast(kast.new_sort, symbol_table, debug=debug)
        oldSortStr = pretty_print_kast(kast.old_sort, symbol_table, debug=debug)
        attStr = pretty_print_kast(kast.att, symbol_table, debug=debug)
    ...
    if type(kast) is KRule or type(kast) is KClaim:
        ...
        attsStr = pretty_print_kast(kast.att, symbol_table, debug=debug)
    ...
    if type(kast) is KContext:
        ...
        attsStr = pretty_print_kast(kast.att, symbol_table, debug=debug)
        if kast.requires != Bool.true:
            requiresStr = pretty_print_kast(kast.requires, symbol_table, debug=debug)
...
def pretty_print_kast_bool(kast, symbol_table, debug=False):
    ...
    if type(kast) is KApply and kast.label.name in ['_andBool_', '_orBool_']:
        clauses = [pretty_print_kast_bool(c, symbol_table, debug=debug) for c in flattenLabel(kast.label.name, kast)]
