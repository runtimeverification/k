...
public class BasicTransformerPropagator implements Transformer {
	...
	public BasicTransformerPropagator(HookedTransformer hookedTransformer, DefinitionHelper definitionHelper) {
	...
	public ASTNode transform(Definition node) throws TransformerException {
		...
		for (DefinitionItem di : node.getItems()) {
			ASTNode result = di.accept(delegate);
			if (result != di)
				change = true;
			if (result != null) {
	...
	public ASTNode transform(Module node) throws TransformerException {
		...
		for (ModuleItem mi : node.getItems()) {
			ASTNode result = mi.accept(delegate);
			if (result != mi)
				change = true;
			if (result != null) {
	...
	public ASTNode transform(Sentence node) throws TransformerException {
		...
		ASTNode bodyAST = body.accept(delegate);
		if (bodyAST != body)
			change = true;
		if (null == bodyAST)
			return null;
		...
		if (condition != null) {
			ASTNode conditionAST = condition.accept(delegate);
	...
	public ASTNode transform(Production node) throws TransformerException {
		...
		for (ProductionItem pi : node.getItems()) {
			ASTNode result = pi.accept(delegate);
			if (result != pi)
				change = true;
			if (result != null) {
	...
	public ASTNode transform(Collection node) throws TransformerException {
		...
		for (Term t : node.getContents()) {
			ASTNode result = t.accept(delegate);
			if (result != t)
				change = true;
			if (result != null) {
				if (!(result instanceof Term)) {
	...
	public ASTNode transform(MapItem node) throws TransformerException {
		...
		ASTNode key = term.accept(delegate);
		if (key == null)
			return null;
		...
		term = node.getValue();
		ASTNode value = term.accept(delegate);
		if (value == null)
			return null;
	...
	public ASTNode transform(KApp node) throws TransformerException {
		...
		ASTNode label = term.accept(delegate);
		if (label == null)
			return null;
		...
		term = node.getChild();
		ASTNode child = term.accept(delegate);
		if (child == null)
			return null;
	...
	public ASTNode transform(Rewrite node) throws TransformerException {
		...
		ASTNode left = term.accept(delegate);
		if (left == null)
			return null;
		...
		term = node.getRight();
		ASTNode right = term.accept(delegate);
		if (right == null)
			return null;
	...
	public ASTNode transform(Attributes node) throws TransformerException {
		...
		for (Attribute at : node.getContents()) {
			ASTNode result = at.accept(delegate);
			if (result != at)
				change = true;
			if (result != null) {
	...
	public String getName() {
		return delegate.getName() + " (propagator)";
	}
