...
let add_identifier: (string -> unit) ref = 
  ref (fun _ -> E.s ("Bug: You called an uninitialized add_identifier")) 

let add_type: (string -> unit) ref = 
  ref (fun _ -> E.s ("Bug: You called an uninitialized add_type")) 

let push_context: (unit -> unit) ref = 
  ref (fun _ -> E.s ("Bug: You called an uninitialized push_context")) 

let pop_context: (unit -> unit) ref = 
  ref (fun _ -> E.s ("Bug: You called an uninitialized pop_context")) 
