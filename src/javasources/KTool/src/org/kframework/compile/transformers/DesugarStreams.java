...
//            syntax List ::= "#buffer" "(" K ")"           [cons(List1IOBufferSyn)]
            TermCons buffer = new TermCons(Sort2.of("Stream"), "Stream1IOBufferSyn", context);
            java.util.List<Term> bufferTerms = new ArrayList<Term>();
            bufferTerms.add(new Variable("$stdin", Sort2.of("String"))); // eq stdinVariable = mkVariable('$stdin,K) .
            buffer.setContents(bufferTerms);
            items.add(newListItem(buffer));

            items.add(new Variable("$noIO", Sort2.LIST));//          eq noIOVariable = mkVariable('$noIO,List) .

//            syntax List ::= "#istream" "(" Int ")"        [cons(List1InputStreamSyn)]
            TermCons stdinStream = new TermCons(Sort2.of("Stream"), "Stream1InputStreamSyn", context);
...
//            | "#ostream" "(" Int ")"        [cons(List1OutputStreamSyn)]
            TermCons stdoutStream = new TermCons(Sort2.of("Stream"), "Stream1OutputStreamSyn", context);
            java.util.List<Term> stdinStreamTerms = new ArrayList<Term>();
