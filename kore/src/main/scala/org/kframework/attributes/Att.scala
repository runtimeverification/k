...
object Att {
  ...
  final val COMM = Key("comm", KeyType.BuiltIn, KeyParameter.Forbidden, Set(classOf[Production], classOf[Rule]))
  ...
  final val STREAM = Key("stream", KeyType.BuiltIn, KeyParameter.Optional, Set(classOf[Production], classOf[Rule]))
