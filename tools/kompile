...
my $maude_path = File::Spec->catfile((File::Basename::fileparse($0))[1], 'kmaude');
...
#    print "Maudification: $language_file_name\n\n";
    ...
    if ($klabels ne "")
    {
		...
		#		print "F: $f\nM: $maudified\n\n";
				if ($maudified =~ /^\s*(in|load).*($prelude)(\.maude)?((\s*?\n)|(\s*\-\-\-.*?\n))/sgm)
				{
					$prelude =~ s/\.maude$//sg;
					$maudified =~ s/^\s*(in|load).*($prelude)(\.maude)?((\s*?\n)|(\s*\-\-\-.*?\n))/in $kshared$+/sgm;
...
# File name where the compiled output will be stored:
	...
	my $postprocessor = File::Spec->catfile($k_tools_dir, "postprocess_texcmds.pl");
