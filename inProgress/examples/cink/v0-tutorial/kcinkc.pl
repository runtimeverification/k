...
# append kmod header
	...
	$output_source .= "\n\tsyntax Pgm ::= p$macro_name\n\n";
	$output_source .= "\tmacro p$macro_name = \n$macro_content\n\nendkm\n";
...
#    close FILE;
    ...
    # call maude
    print "$kompile_path -pgm $macro_name.k -cmod $cmod -pmod $module_name -pname p$macro_name\n";
    my $status = system("$kompile_path -pgm $macro_name.k -cmod $cmod -pmod $module_name -pname p$macro_name  >$output_file 2>$error_file");
    if (($status >>= 8) != 0)
    {
