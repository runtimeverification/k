...
char* parse(const char* parse_table_name,
	...
	if (!parse_tree) printf("\tExternal sglr library: Error parsing\n");
	...
	if (!parseTree) printf("\tExternal sglr library: Error adding Position Info\n");
	parse_tree = implode(parseTree);
	if (!parse_tree) printf("\tExternal sglr library: Error imploding\n");
	fflush(stdout);
