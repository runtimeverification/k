...
abstract class TransformerWithErrors[E] {
  ...
  private[this] val generalTransformer = new GeneralTransfomer[E, Unit] {
    ...
    override def apply(a: Ambiguity): (Either[E, Term], Unit) = (outerThis.apply(a), warningUnit)
    override def apply(tc: TermCons): (Either[E, Term], Unit) = (outerThis.apply(tc), warningUnit)
    override def apply(kl: KList): (Either[E, Term], Unit) = (outerThis.apply(kl), warningUnit)
    override def apply(c: Constant): (Either[E, Term], Unit) = (outerThis.apply(c), warningUnit)
  }
  ...
  def apply(a: Ambiguity): Either[E, Term] = generalTransformer.mapChildren(a)._1
  def apply(tc: TermCons): Either[E, Term] = generalTransformer.mapChildrenStrict(tc)._1
  def apply(kl: KList): Either[E, Term] = generalTransformer.mapChildrenStrict(kl)._1
