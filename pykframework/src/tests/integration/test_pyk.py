...
from pykframework.__main__ import main
from pykframework.testing import KompiledTest
...
class TestGraphImports(KompiledTest):
    ...
    def test_graph_imports(self, assume_argv: AssumeArgv, definition_dir: Path) -> None:
        ...
        assume_argv(['pykframework', 'graph-imports', str(definition_dir)])
...
class TestMinimizeTerm(KompiledTest):
    ...
    def test_minimize_term(self, assume_argv: AssumeArgv, tmp_path: Path, definition_dir: Path) -> None:
        ...
        main()
        assume_argv(['pykframework', 'print', str(definition_dir), str(prove_res_file), '--output-file', str(actual_file)])
        main()
...
class TestRpcPrint(KompiledTest):
    ...
    def test_rpc_print(self, assume_argv: AssumeArgv, tmp_path: Path, definition_dir: Path, filename_stem: str) -> None:
        # Given
        input_file = TEST_DATA_DIR / 'pykframework-rpc-print' / f'{filename_stem}.json'
        expected_file = TEST_DATA_DIR / 'pykframework-rpc-print' / f'{filename_stem}.pretty.golden'
        actual_file = tmp_path / f'{filename_stem}.pretty'
        assume_argv(['pykframework', 'rpc-print', str(definition_dir), str(input_file), '--output-file', str(actual_file)])
...
class TestRpcKast:
    ...
    def test_rpc_kast(
        ...
        # Given
        reference_file = TEST_DATA_DIR / 'pykframework-rpc-kast' / f'{reference_filename_stem}.json'
        input_file = TEST_DATA_DIR / 'pykframework-rpc-kast' / f'{input_filename_stem}.json'
        expected_file = TEST_DATA_DIR / 'pykframework-rpc-kast' / f'{output_filename_stem}.json.golden'
