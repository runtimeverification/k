...
public class CheckAtt {
    ...
    private void check(Production prod) {
        if (!prod.sort().equals(Sorts.KItem())) {
            ...
            if (sortAtt.contains(Att.HOOK()) && !sortAtt.get(Att.HOOK()).equals("ARRAY.Array")
                ...
                if (!prod.att().contains(Att.FUNCTION()) && !prod.att().contains(Att.BRACKET()) &&
                        !prod.att().contains("token") && !prod.att().contains("macro")
                        && !(prod.klabel().isDefined() && macros.contains(prod.klabel().get()))) {
                    if (!(prod.sort().equals(Sorts.K())
                            && ((prod.klabel().isDefined() && (prod.klabel().get().name().equals("#EmptyK")
                                    || prod.klabel().get().name().equals("#KSequence"))) || prod.isSubsort()))) {
                        if (!(sortAtt.contains("cellCollection") && prod.isSubsort())) {
                            errors.add(KEMException
                                    .compilerError("Cannot add new constructors to hooked sort " + prod.sort(), prod));
                        }
                    }
                }
            }
        ...
        if (prod.att().contains("binder") && !isSymbolicKast) {
            if (!prod.att().get("binder").equals("")) {
                errors.add(
                        KEMException.compilerError("Attribute value for 'binder' attribute is not supported.", prod));
            }
            ...
            } else if (!m.sortAttributesFor().get(prod.nonterminals().apply(0).sort().head())
                    .getOrElse(() -> Att.empty()).getOptional(Att.HOOK()).orElse("").equals("KVAR.KVar")) {
            ...
            for (int i = 0; i < format.length(); i++) {
                ...
                if (c == '%') {
                    if (i == format.length() - 1) {
                        errors.add(KEMException.compilerError("Invalid format attribute: unfinished escape sequence.",
                                prod));
                        break;
                    }
                            ...
                            if (idx == 0 || idx > prod.items().size()) {
                                errors.add(
                                        KEMException.compilerError(
                                                "Invalid format escape sequence '%" + sb.toString()
                                                        + "'. Expected a number between 1 and " + prod.items().size(),
                                                prod));
                            } else {
        ...
        } else if (!prod.att().contains("token") && !prod.sort().equals(Sorts.Layout())
        ...
        if (hasColors && nescapes + nterminals != ncolors) {
            errors.add(KEMException.compilerError("Invalid colors attribute: expected " + (nescapes + nterminals)
                    + " colors, found " + ncolors + " colors instead.", prod));
        }
        ...
        if (prod.att().contains(Att.TOTAL()) && !prod.att().contains(Att.FUNCTION())) {
            errors.add(KEMException.compilerError(
                    "The attribute 'total' was applied to a production which does not have the 'function' attribute; the attribute was ignored.",
                    prod));
        }
    }
    ...
    private void check(Rule rule) {
        if (rule.isMacro()) {
            kem.registerCompilerWarning(ExceptionType.FUTURE_ERROR, errors,
                    "The attribute [" + rule.att().getMacro().get()
