...
/**
 * Expands all the macros in a particular module. A macro is a rule (without a side condition) which
 * is tagged with the "macro" attribute. This class assumes that all such macros perform no complex
 * matching on the left hand side of the rule, and generates a simple substitution and recursively
 * applies it.
 */
public class ExpandMacros {
  ...
  private ExpandMacros(
      ...
      boolean sentences) {
    this(
        sentences ? new ResolveFunctionWithConfig(mod) : null,
        mod,
        files,
        kem,
        kompileOptions,
  ...
  public ExpandMacros(
    ...
    macros =
        allMacros.stream()
            ...
            .collect(Collectors.groupingBy(r -> ((KApply) getLeft(r, reverse)).klabel()));
    macrosBySort =
        stream(mod.allSorts())
            .collect(
                Collectors.toMap(
  ...
  private boolean isMacro(Att att, boolean reverse) {
    return att.contains(Att.ALIAS_REC())
        || att.contains(Att.ALIAS())
        || (!reverse && (att.contains(Att.MACRO()) || att.contains(Att.MACRO_REC())));
  }
  ...
  private RuleOrClaim expand(RuleOrClaim rule) {
    ...
    RuleOrClaim result =
        rule.newInstance(
            expand(rule.body()), expand(rule.requires()), expand(rule.ensures()), rule.att());
  ...
  private Context expand(Context context) {
    ...
    Context result = Context(expand(context.body()), expand(context.requires()), context.att());
  ...
  private Sentence check(Sentence s) {
    ...
    /**
     * At this point, all macros inside rules should have been expanded. However, a user could have
     * accidentally left out a rule that expands the macro or incorrectly programmed this rule. If
     * this is the case, rules may contain macros that have not been expanded.
     */
      ...
      VisitK visitor =
          new VisitK() {
            ...
            public void apply(KApply k) {
              ...
              if (atts.isDefined()
                  && mod.attributesFor().apply(k.klabel()).getMacro().isDefined()) {
                errors.add(
                    KEMException.compilerError(
                        "Rule contains macro symbol that was not expanded", s));
              }
    ...
    if (!errors.isEmpty()) {
      ...
      throw KEMException.compilerError(
          "Had " + errors.size() + " structural errors after macro expansion.");
    }
    return s;
  }

  public K expand(K term) {
    if (macros.size() == 0 && macrosBySort.size() == 0) return term;
    FileLock lock = null;
    if (cover) {
      try {
        lock = channel.lock();
      ...
      K result =
          new TransformK() {
            ...
            private <T extends K> K applyMacros(T k, List<Rule> rules, Function<T, K> superApply) {
              if (rules == null) return superApply.apply(k);
              K applied = superApply.apply(k);
              for (Rule r : rules) {
                if (!r.requires().equals(BooleanUtils.TRUE)) {
                  throw KEMException.compilerError(
                      "Cannot compute macros with side conditions.", r);
                }
        ...
        if (!sorts.contains(null)) {
          prods.removeIf(
              p ->
                  !hasPolyAtt(p, idx)
                      && sorts.stream()
                          .noneMatch(s -> mod.subsorts().lessThanEq(s, p.nonterminal(idx).sort())));
        }
      }
  ...
  private boolean match(Map<KVariable, K> subst, K pattern, K subject, RuleOrClaim r) {
    ...
    if (pattern instanceof KApply p && subject instanceof KApply s) {
      ...
      if (!p.klabel().name().equals(s.klabel().name())) {
        if (!mod.overloads()
            .greaterThan(
                mod.productionsFor().apply(p.klabel()).head(),
                mod.productionsFor().apply(s.klabel()).head())) {
          return false;
        }
      }
    ...
    throw KEMException.compilerError(
        "Cannot compute macros with terms that are not KApply, KToken, or KVariable.", r);
  }
