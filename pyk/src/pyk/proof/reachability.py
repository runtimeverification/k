...
class APRProver(Prover[APRProof, APRProofStep, APRProofResult]):
    ...
    def _check_subsume(self, node: KCFG.Node, target_node: KCFG.Node, proof_id: str) -> CSubst | None:
        ...
        if self.fast_check_subsumption and not self._may_subsume(node, target_node):
            ...
            return None
        _csubst = self.kcfg_explore.cterm_symbolic.implies(
            node.cterm, target_cterm, booster_implies=self.booster_implies
        )
    ...
    def failure_info(self, proof: APRProof) -> FailureInfo:
        return APRFailureInfo.from_proof(
            proof, self.kcfg_explore, counterexample_info=self.counterexample_info, booster_implies=self.booster_implies
        )
...
class APRFailureInfo(FailureInfo):
    ...
    @staticmethod
    def from_proof(
        proof: APRProof, kcfg_explore: KCFGExplore, counterexample_info: bool = False, booster_implies: bool = False
    ) -> APRFailureInfo:
        ...
        for node in proof.failing:
            ...
            _, reason = kcfg_explore.implication_failure_reason(
                node_cterm, target_cterm, booster_implies=booster_implies
            )
