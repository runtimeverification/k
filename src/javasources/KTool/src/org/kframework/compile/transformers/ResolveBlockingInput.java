...
public class ResolveBlockingInput extends GetLhsPattern {
    ...
    private Map<String, String> delimiterOfInputCell = new HashMap<>();
    private java.util.List<Rule> generatedRules = new ArrayList<Rule>();
    
    /**
     * Indicates whether the current visiting Rule has input cell.
     */
    private boolean hasInputCell;
    private Term resultCondition;
    ...
    public ASTNode visit(Definition node, Void _)  {
        /* collect input cells and their delimiters in the configuration */
        Configuration config = MetaK.getConfiguration(node, context);
        new BasicVisitor(context) {
            @Override
            public Void visit(Cell node, Void _) {
                String stream = node.getCellAttribute("stream");
                if ("stdin".equals(stream)) {
                    String delimiter = node.getCellAttribute("delimiters");
                    if (delimiter == null) {
                        delimiter = " \n\t\r";
                    }
                    delimiterOfInputCell.put(node.getLabel(), delimiter);
                }
    ...
    public ASTNode visit(Module node, Void _)  {
        ...
        /* append generated rules to the end of the module */
        if (generatedRules.isEmpty()) {
            return node;
        }
        node = node.shallowCopy();
        node.getItems().addAll(generatedRules);
        return node;
    }
    ...
    public ASTNode visit(Rule node, Void _)  {
        ...
        /* skip rules auto-generated by AddStreamCells pass */
        ...
        ASTNode result = super.visit(node, _);
        if (hasInputCell) {
            /* generate a new rule and append new side-condition to the current rule */
            // TODO(YilongL): to be more specific about the effect
            generatedRules.add((Rule) result);
    ...
    public ASTNode visit(Cell node, Void _)  {
        if ((!delimiterOfInputCell.containsKey(node.getLabel()))) {
            return super.visit(node, _);
        }
        ...
        Term parseTerm = KApp.of(parseInputLabel, 
            ...
            StringBuiltin.kAppOf(delimiterOfInputCell.get(node.getLabel())));
...
//        ctor(List)[replaceS[emptyCt(List),parseTerm(string(Ty),nilK)],ioBuffer(mkVariable('BI,K))]
    ...
    /**
     * Helper method to create predicate of the form
     * {@code ``isStream(var) =/=K true''}.
     * 
     * @param var
     *            the variable to be put in the predicate
     * @return the result predicate term
     */
