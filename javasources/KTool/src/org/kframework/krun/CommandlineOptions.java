...
public class CommandlineOptions {
	...
	public void initializeKRunOptions() {
		...
		addOptionS(OptionBuilder.withLongOpt("io").withDescription("Use real IO when running the definition. (Default: enabled).").create());
		addOptionS(OptionBuilder.withLongOpt("no-io").withDescription("").create());
		...
		addOptionS(OptionBuilder.withLongOpt("pattern").hasArg().withArgName("string").withDescription("Specify a term and/or side condition that the result of execution or search must match in order to succeed. Return the resulting matches as a list of substitutions.rch must match in order to succeed. Return the resulting matches as a list of substitutions. In conjunction with it you can specify other 2 options that are optional: bound (the number of desired solutions) and depth (the maximum depth of the search).").create());
		...
		addOptionS(OptionBuilder.withLongOpt("output-mode").hasArg().withArgName("mode").withDescription("How to display Maude results. <mode> is either [pretty|raw|binary|none]. (Default: pretty).").create());
