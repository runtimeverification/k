...
public class ResolveFresh extends CopyOnWriteTransformer {
    ...
    public ASTNode visit(Sentence node, Void _)  {
        ...
        Variable freshVar = Variable.getFreshVar(Sort2.of("Int"));
        ...
        TermCons t = new TermCons(Sort2.of("Int"), "Int1PlusSyn", context);
    ...
    private Substitution freshSubstitution(
        ...
        for (Variable var : vars) {
            TermCons idxTerm = new TermCons(Sort2.of("Int"), MetaK.Constants.plusIntCons, context);
            List<Term> subterms = idxTerm.getContents();
            subterms.add(idxVar);
            subterms.add(IntBuiltin.kAppOf(idx));
            ++idx;

            Sort2 sort = var.getSort();
