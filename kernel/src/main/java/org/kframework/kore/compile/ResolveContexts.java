...
public class ResolveContexts {
    ...
    private Stream<? extends Sentence> resolve(Context context, Module input) {
        ...
        // Find a hole with sort casting (mostly generated by ResolveStrict.resolve)
        // e.g., context HOLE:Exp + _:Exp
        K holeWithSemanticCast = new VisitKORE() {
            K holeWithSemanticCast;
            public K process(K k) {
                apply(k);
                return holeWithSemanticCast;
            }
            @Override
            public Void apply(KApply k) {
        ...
        if (heated == null) {
            if (kompileOptions.strict() && holeWithSemanticCast != null) {
                // Preserve sort casting if any
                heated = holeWithSemanticCast;
            } else {
