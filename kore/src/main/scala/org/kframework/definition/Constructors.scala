...
object Constructors {
  ...
  def Production(sort: Sort, items: Seq[ProductionItem]) = definition.Production(sort, items, Attributes())
  def Production(sort: Sort, items: Seq[ProductionItem], att: Attributes) = definition.Production(sort, items, att)
