...
import org.kframework.backend.maude.MaudeContextExtractor;
...
public class KompileBackend extends BasicBackend {
	...
	public Definition lastStep(Definition def) {
		...
		if (GlobalSettings.verbose)
			sw.printIntermediate("Writing the rules to Maude.");
		MaudeContextExtractor contextExtractor = new MaudeContextExtractor();
		try {
			def = (Definition) def.accept(contextExtractor);
		} catch (TransformerException e) {
		}
		String contexts = "mod " + mainModule + "-CONTEXTS is\n" + " including " + mainModule + "-BASE .\n" + contextExtractor.getResult() + "endm\n";
		FileUtil.saveInFile(DefinitionHelper.dotk.getAbsolutePath() + "/contexts.maude", contexts);
		if (GlobalSettings.verbose)
			sw.printIntermediate("Writing the contexts to Maude.");
		return def;
	}
	...
	public void run(Definition javaDef) throws IOException {
		...
		String definition = "load \"" + KPaths.windowfyPath(DefinitionHelper.dotk.getAbsolutePath() + "/base.maude") + "\"\n"
				+ "load \"" + KPaths.windowfyPath(DefinitionHelper.dotk.getAbsolutePath() + "/contexts.maude") + "\"\n";
		...
		final String mainModule = javaDef.getMainModule();
		final String cxtModule = mainModule + "-CONTEXTS";
		String compile = load
				+ definition
				// + maudeFilter.getResult()
				+ " load \"" + KPaths.getKBase(true) + "/bin/maude/compiler/all-tools\"\n" + "---(\n" + "rew in COMPILE-ONESHOT : partialCompile('" + cxtModule + ", '" + step + ") .\n" + "quit\n"
				+ "---)\n" + " loop compile .\n" + "(compile " + cxtModule + " " + step + " transitions " + transition + " superheats " + superheat + " supercools " + supercool
		...
		compiled = compiled.substring(start, enddd).replaceFirst(cxtModule, mainModule + "-STRICT");

		String defFile = javaDef.getMainFile().replaceFirst("\\.[a-zA-Z]+$", "");
		FileUtil.saveInFile(DefinitionHelper.dotk.getAbsolutePath() + "/strict.maude", compiled);
		...
		String main = load + "load \".k/base.maude\"\n"
		        ...
		        + "  including " + mainModule + "-STRICT .\n"
				+ "endm\n";
		FileUtil.saveInFile(defFile + "-compiled.maude", main);

		if (start == -1 || enddd == -1) {
			KException exception = new KException(ExceptionType.ERROR,
					KExceptionGroup.CRITICAL,
					"Incomplete output generated by the compiler. Check the '" + defFile + "-compiled.maude'.",
					"top level",
					"Maude compilation");
