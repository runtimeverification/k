...
import org.kframework.compile.TestConfiguration;
...
public class AddParentsCellsTest {
  ...
  public void testMultiplicityShared() {
    ...
    K expected =
        cell("<ts>", cell("<t>", cell("<k>", intToToken(1)), cell("<env>", intToToken(2))));
    Assert.assertEquals(expected, pass.concretizeCell(term));
  }
  ...
  public void testDeep2() {
    Assert.assertEquals(
        Lists.newArrayList(cell("<ts>", cell("<t>", intToToken(1)), cell("<t>", intToToken(2)))),
        pass.makeParents(
            KLabel("<ts>"),
            false,
            Lists.newArrayList(cell("<t>", intToToken(1)), cell("<t>", intToToken(2)))));
  }
  ...
  public void testRewrites() {
    K term =
        cell("<T>", cell("<k>", intToToken(1)), KRewrite(cell("<k>", intToToken(2)), cell("<k>")));
    K expected =
        cell(
            "<T>",
            cell(
  ...
  public void testRewriteWithCells() {
    K term =
        cell(
            "<T>",
            cell("<k>", intToToken(1)),
            KRewrite(cells(cell("<k>", intToToken(2)), cell("<msg>")), cell("<k>")));
    K expected =
        cell(
            "<T>",
            cell(
  ...
  public void testEmptySide() {
    ...
    K expected =
        cell("<T>", cell("<ts>", cell("<t>", cell("<k>"), KRewrite(cell("<msg>"), cells()))));
    Assert.assertEquals(expected, pass.concretizeCell(term));
  }
  ...
  public void testDotsTogether() {
    ...
    K expected =
        cell(
            ...
            true,
            true,
            cell("<t>", true, true, cell("<k>", intToToken(0)), cell("<env>", intToToken(2))));
    Assert.assertEquals(expected, pass.concretizeCell(term));
  }
    ...
    K expected =
        cell(
            ...
            cell(
                ...
                cell(
                    "<t>",
                    cell("<msg>", intToToken(0)),
                    cell("<msg>", cell("<msgId>", intToToken(1)))),
                cell("<t>", cell("<k>", intToToken(6))),
                cell(
                    "<t>",
                    cell("<k>", intToToken(2)),
  ...
  public void testNonCellItem() {
    ...
    K expected =
        cell(
            "<T>",
            cells(KApply(KLabel(".K")), cell("<ts>", cell("<t>", cell("<k>", KVariable("X"))))));
    Assert.assertEquals(expected, pass.concretize(term));
  }
  ...
  public void testNonCellItemRewrite() {
    K term =
        cell(
            "<T>",
            KRewrite(
                KApply(KLabel("label")), cells(KApply(KLabel(".K")), cell("<k>", KVariable("X")))));
