...
public class XMLReader {
    ...
    public void read(InputStream is, Document d, int pos) throws IOException, BadLocationException{
        ...
        try {
            ...
            DefaultStyledDocument.ElementSpec spec=new DefaultStyledDocument.ElementSpec(tagAttrs, DefaultStyledDocument.ElementSpec.EndTagType);
            specs.add(spec);
...
//            >
                ...
                if (n instanceof Element) {
                    ...
                    for(int index = 0 ; index <lines.length; index++){                    
                    	if(!lines[index].equals("")){
                    		...
                    		spec=new DefaultStyledDocument.ElementSpec(tagRowStartAttrs, DefaultStyledDocument.ElementSpec.StartTagType);
			                specs.add(spec);
			                spec=new DefaultStyledDocument.ElementSpec(XMLDocument.PLAIN_ATTRIBUTES, DefaultStyledDocument.ElementSpec.ContentType,lines[index].toCharArray(), 0, lines[index].length());
			                specs.add(spec);
            ...
            if (node instanceof org.w3c.dom.Document) {
                spec=new DefaultStyledDocument.ElementSpec(ColorTagMap.getColorForTag(node.getNodeName()), DefaultStyledDocument.ElementSpec.ContentType," ".toCharArray(), 0, 1);
                specs.add(spec);
            }
            else {
    //            </
                ...
                specs.add(spec);
    //            tag name
                    spec=new DefaultStyledDocument.ElementSpec(ColorTagMap.getColorForTag(node.getNodeName()), DefaultStyledDocument.ElementSpec.ContentType,node.getNodeName().toCharArray(), 0, node.getNodeName().length());
                    specs.add(spec);
    //            />
                spec=new DefaultStyledDocument.ElementSpec(XMLDocument.BRACKET_ATTRIBUTES, DefaultStyledDocument.ElementSpec.ContentType,">\n".toCharArray(), 0, 2);
                specs.add(spec);
