...
the entry point to the parser, in this case the `K` sort. In many situations it
is useful to cast to other sorts there as well. The sort used for the `$PGM`
variable is is referred to as the start symbol. During parsing, The default
start symbol `K` subsumes all user-defined sorts except for syntactic lists.
