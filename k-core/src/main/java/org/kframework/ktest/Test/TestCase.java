...
public class TestCase {
    ...
    public boolean isDefinitionKompiled() {
        return new File(getKompileDirFullPath()).isDirectory();
    }
    ...
    /**
     * Search for -d or --directory argument in kompile arguments of test case.
     ...
     * file)
     * @return Full path of directory that'll have -kompiled directory. null if -d is not provided.
     */
    private String getKompileDirArg() {
    ...
    /**
     * Returns full path of -kompiled directory that'll be generated by this test case.
     * @return Full path of -kompiled directory.
     */
    private String getKompileDirFullPath() {
        String dirArg = getKompileDirArg();
        String dashKompiled = FilenameUtils.getBaseName(definition.getObj()) + "-kompiled";
        if (dirArg != null) {
            return FilenameUtils.concat(dirArg, dashKompiled);
        } else {
            return FilenameUtils.concat(
            ...
            if (pgmFile.isFile()) {
                ...
                if (extensions.contains(FilenameUtils.getExtension(pgmFilePath))) {
                    ...
                    if (!hasDirectory) {
                        ...
                        String kompileDir = getKompileDirArg();
                        if (kompileDir == null) {
