...
/**
 * refreshes variable names in terms and sentences according to alpha equivalence. Variables that
 * have previously been refreshed are not normalized on succeeding passes, allowing the user to
 * fine-tune the applyRefresh process such that arbitrary subterms can share a common prefix.
 */
public class RefreshRules {
   ...
   * @return The refreshed version of {@code rule}, in which each variable is alpha-renamed to fresh
   *     variable. name.
   */
  ...
  public static Collection<Rule> refresh(Collection<Rule> rules, Set<String> avoidVars) {
    ...
    return rules.stream()
        .map(refreshRules::applyRefreshResetVars)
  ...
  private K applyRefresh(K term) {
    return new TransformK() {
      ...
      public K apply(KVariable var) {
        if (var.att().contains(Att.REFRESHED())) return var;
