...
class KCFGTestCase(TestCase):
    ...
    def test_pretty_print(self):
        ...
        self.assertMultiLineEqual(actual,
                                  ...
                                  f"┃  KApply(label=KLabel(name='_==K_', params=()), args=(KVariable(name='x'), KToken(token='6', sort=KSort(name='Int'))))\n"
                                  f"┃\n"
                                  f"┣━ {_short_hash(5)} (expanded)\n"
                                  f"┃  KApply(label=KLabel(name='_==K_', params=()), args=(KVariable(name='x'), KToken(token='5', sort=KSort(name='Int'))))\n"
                                  f"┃   │  (1 step)\n"
                                  f"┃   ├  {_short_hash(2)} (expanded)\n"
                                  f"┃   ┊ (looped back)\n"
                                  f"┃\n"
                                  ...
                                  f"┃  KApply(label=KLabel(name='_==K_', params=()), args=(KVariable(name='x'), KToken(token='4', sort=KSort(name='Int'))))\n"
                                  f"┃   │  (1 step)\n"
                                  f"┃   ├  {_short_hash(5)} (expanded)\n"
                                  f"┃   ┊ (continues as previously)\n"
                                  f"┃\n"
                                  f"┗━ {_short_hash(7)} (expanded)\n"
                                  f"   KApply(label=KLabel(name='_==K_', params=()), args=(KVariable(name='x'), KToken(token='7', sort=KSort(name='Int'))))\n"
                                  f"    │  (1 step)\n"
                                  f"    └  {_short_hash(6)} (target, leaf)\n"
                                  f"\n"
