...
__all__ = ['pykframeworkBackend', 'kompile']
...
class pykframeworkBackend(Enum):
    LLVM = 'llvm'
    HASKELL = 'haskell'
    KORE = 'kore'
    MAUDE = 'maude'
...
def kompile(
    main_file: str | Path,
    *,
    backend: str | pykframeworkBackend | None = None,
    # ---
    ...
    **kwargs: Any,
    ...
    pykframework_backend = pykframeworkBackend(backend) if backend else None
    if pykframework_backend is pykframeworkBackend.BOOSTER:
    ...
    kwargs['backend'] = KompileBackend(pykframework_backend.value) if pykframework_backend else None
